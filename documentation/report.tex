...
And by \term{scale}, I don't just mean \term{supports a large (or unpredictable) volume of users}.
...
But how can we create one whenever we want to run our tests?
...
By the way, if merged commits automatically get deployed (see TODO: sec:CD) then commits are now not just a representation of your application's behavior but also a representation of exactly what you are offering your user.
...
If you wanted to run your application on multiple servers~\footnotemark{} you wouldn't be able to, because each server would be tracking a different number. \footnotetext{
...
The framework provides a library of classes and functions for describing infrastructure in a familiar language like TypeScript, Python, or Java, and tools to turn your code into a representation template that is read by a classical system like CloudFormation.
...
\begin{enumerate}
  ...
  \item The software engineer takes back control of their own infrastrcture from the DevOps people by writing in a language they are already familiar with.
\end{enumerate}
...
How much time do we save by writing our infrastructure using CDK vs CloudFormation?
...
It's a good thing when you only have to mantain 20\% as much code, but that's not the only advantage that IaC provides.
...
It's output should be the addresses of the resources that were actually deployed (whether on a public or private network).
...
Well, there's no plan in place for deploying new versions of the backend, or for recovering the application should the server fail, but that's the subject of section \#\#\#.
...
For example, here's a sample user interaction with Marcus, where user's commands look like \user{this} and Marcus's replies look like \marcus{this} (explanation on the right).
...
\begin{enumerate}
  ...
  \item An API Gateway to expose the lambda through a public HTTP endpoint.
...
I can't stress how easy it is to develop and deploy this application.
...
CDK is smart - if the bucket's configuration doesn't change, it will simply remain during a new deploy.
...
There are no bare EC2 isntances or ECS containers running Jongleur code.
