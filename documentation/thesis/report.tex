...
\begin{abstract}
  ...
  Of particular note is Jongleur, a web application for musicians that ingests and catalogs audio recordings, but we approach this capstone project by working our way through a couple of interesting cloud apps.
\end{abstract}
...
In section 2, we'll propose a list of design principles for architecting cloud software, in sections 3 through 9 we'll explore those patterns, and in sections 10 through 12 we'll apply them to real projects.
...
\begin{enumerate}
  ...
  \item Serverlessness
\end{enumerate}
Later, to demonstrate each one, we'll look at three example applications that display these finer details of software development.
...
Sometimes the tests we write turn out to be flaky, meaning that they behave inconsistently even when the code doesn't change.
Flakiness is a big deal!
Consider that Google has a team dedicated to understanding and monitoring flaky tests~\cite{google-flaky-blog} and they've written extensively about their research into detecting the causes of flakiness.~\cite{google-deflake}
...
This is the meaning of \term{infrastructure as code}, discussed in section 5.
...
If you write code later that introduces a bug, you will always be able to revert to a previous commit and expect the exact same (working) behavior that you had before, no exceptions.

By the way, if merged commits automatically get deployed then commits are now not just a representation of your application's behavior but also a representation of exactly what your users are seeing.
...
But this only works if deploying an old version of your app doesn't cause loss of valuable state that's been built up as the program has been running.
If you can limit the influence of state on your app, there won't be an issue.
...
\infobox{Horizontal scaling}{
  ...
  You no longer have a single point of failure, so if a machine goes down, traffic can be transparently directed to others.
  Also, your app can dynamically scale by automatically launching or terminating instances in response to load.
}

To summarize, the particular server that a user's session lives on should be functionally transparent to the user.
(In section 7, we take this to the extreme and discuss how to make the server transparent to the developer as well.)
...
So in general, any local state that you keep should be ephemeral; it's hard to back up state that's kept on the same machine as your application code, but that's okay if the data doesn't need to be backed up.
...
Infrastructure as code really helps in this regard because a stack will deploy the same way every time.
...
You've just tested a complete version of your app, exactly like your users will see it.
...
\infobox{More on remote tests}{
  ...
  You may need to scale down the resources for each test deployment to save costs, especially if your tests take a long time to run.
  But remember that every change you make will set your test environment apart from production so make changes sparingly.
}
...
As for the code itself, they'll have to find a way to get the code up and running automatically during deployment if they want to have a really tight development and iteration pipeline.
...
The developer just wanted to expose some functions over the internet.
...
So while serverless applications \textit{may} forgo state and probably \textit{should} in many instances, it is not a guarantee.
On the other hand, it's entirely possible for serverful applications to be stateless.~\cite{xkcd-1}
...
Anywhere you might use your phone's voice assistant ("add an event/reminder to my calendar today at 6PM") you might wish to use Marcus instead.
...
Furthermore, since Lambda scales in response to load automatically, you don't need to think about your app throttling.
...
Second, we create a TypeScript Lambda to resolve SMS queries.
...
Many cloud applications are web apps which interface with the user through the browser.
...
Later we'll discuss Jongleur which uses this approach.
...
The frontend uses Remix,~\cite{remix} a new React framework (it went open source just this past Thanksgiving).
...
How should we scope out and design this app?
...
This will also help in programming the backend resolver because it is also written in TypeScript so the generated types will let us know exactly what fields to expect from the data stores.
...
Because of this, we can't assume that our Lambdas will be able to fit the entirety of any particular audio file in memory at once.
...
We want the user to be able to upload a file directly to our audio intake S3 bucket, using the browser's native capabilities for multi-part background uploads.
...
First, the user will request a \term{presigned url} from the backend, which is an upload target to a specific spot in S3 that is associated with their account.
Then, using this URL (before it expires), the user will make the upload directly to the bucket.
...
Third, record that we are done (either successfully or with an error).
...
As mentioned previously, our destination S3 bucket will be hooked up to CloudFront so our backend can return authenticated URLs to user audio content that stream to the user's device extremely quickly.

\subsubsection{Web}

The Remix web app contains frontend TypeScript and data loaders which technically run on the backend and make requests to our GraphQL API.
...
You can launch the full stack with one command~\cite{jongleur} and view a live version of the app at https://www.jongleur.app/.
...
I've learned firsthand that giving special attention to the way an app is built is the best way to ensure that the pace of development does not slow.

When you write software without regard for proper design, your project will not outlast you.
Do your future collaborators a favor and allow them to build off your work, not replace it.

\section{Acknowledgments}

I want to express genuine appreciation to Dr. Jérémie Lumbroso for advising me on this project and giving me the freedom to take it in the direction that most appealed to me.
...
\screenshottwo{marcus-demo}{Sample interaction with Marcus.}
